...
import { Building2, Sparkles, ArrowRight, Loader2, ShieldCheck, Globe, CheckCircle2 } from 'lucide-react';
...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      className="max-w-6xl mx-auto px-4 sm:px-8 py-8 sm:py-10 space-y-8 sm:space-y-10"
    >
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">AI Visibility Overview</h1>
          <p className="text-slate-400 text-xs sm:text-sm mt-1">Measure how AI search engines position your business across target domains.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        <motion.div
          whileHover={{ y: -3 }}
          className="glass-card p-6 rounded-2xl space-y-2 border border-slate-800"
        >
        ...
        >
          ...
          <p className="text-xs text-slate-500">Active with automated model failover</p>
        </motion.div>
        ...
        >
          ...
          <p className="text-xs text-slate-500">Deterministic evidence extractions</p>
        </motion.div>
      </div>

      {/* Quick Start Audit Banner */}
      <div className="glass-card p-6 sm:p-8 rounded-3xl space-y-4 border border-cyan-500/20 bg-gradient-to-r from-slate-900 via-slate-900 to-slate-950 shadow-xl">
        <div className="space-y-1">
          <h3 className="text-lg font-bold text-white">Instant AI Audit Launcher</h3>
          <p className="text-slate-400 text-xs">Enter any business domain to evaluate how AI search engines recommend your product.</p>
        </div>
        ...
        <form onSubmit={handleQuickAudit} className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold px-6 py-3 rounded-xl text-sm transition-all disabled:opacity-50 active:scale-98"
          >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-white tracking-tight">Recent Target Projects</h2>
        ...
        {isLoading ? (
          <div className="glass-card p-8 rounded-2xl text-center space-y-2">
            ...
            <p className="text-slate-400 text-xs">Loading target projects...</p>
          </div>
        ) : projects.length === 0 ? (
          ...
          <div className="grid gap-3">
            {projects.slice(0, 5).map((proj) => (
              <motion.div
                ...
                className="glass-card p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-slate-800 hover:border-slate-700 transition-all"
              >
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-400">Registered {new Date(proj.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-white text-xs font-semibold px-4 py-2.5 rounded-xl transition-all"
                  >
